...
public class RectangleMiddleMethod extends Method {
    ...
    public double calculateIntegral(Functions function, double left, double right, int n) {
        ...
        double integral = 0;
        for (int i = 0; i <= n; i++) {
            if (!Double.isFinite(function.getFunction().apply(left + i * h - 1 / 2 * h))) {
                double leftLimit = function.getFunction().apply(left + i * h - 1 / 2 * h + DX);
                double rightLimit = function.getFunction().apply(left + i * h - 1 / 2 * h - DX);
            ...
            integral += h * function.getFunction().apply(left + i * h - 1 / 2 * h);
        }
        return integral;
    }
}
